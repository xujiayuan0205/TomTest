...
def compute_metrics(
    ...
    # 二级指标：按 Meta.dimension[0] 分组
    dimension_metrics: Dict[str, Dict[str, int]] = {}
    for is_correct, row in zip([r["is_correct"] for r in per_sample_results], data):
        # 获取 dimension
        dimension = "unknown"
        if row.get("Meta") and isinstance(row["Meta"], dict):
            dimension_value = row["Meta"].get("dimension")
            if isinstance(dimension_value, list) and dimension_value:
                dimension = str(dimension_value[0])
            elif dimension_value:
                dimension = str(dimension_value)

        if dimension not in dimension_metrics:
            dimension_metrics[dimension] = {"correct": 0, "total": 0}
        dimension_metrics[dimension]["total"] += 1
        if is_correct:
            dimension_metrics[dimension]["correct"] += 1

    accuracy = correct / total if total else 0

    # 计算各 dimension 准确率
    secondary_metrics = {
        f"by_dimension.{dim}": stats["correct"] / stats["total"]
        for dim, stats in dimension_metrics.items()
    }
    ...
    return {
        ...
        **secondary_metrics,
        "by_dimension": {
            dim: stats["correct"] / stats["total"]
            for dim, stats in dimension_metrics.items()
        },
        "dimension_counts": {dim: stats["total"] for dim, stats in dimension_metrics.items()},
